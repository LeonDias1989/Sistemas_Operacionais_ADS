...
public class Circular {
	...
	public Processo getProximoProcessoAExecutar() {
		...
		Collections.sort(filaProcessos, new ProcessoComparator(
				ProcessoTipoOrdenacao.PRIORIDADE, false));

		// como ordenei de forma decrescente conforme o parametro false da
		// classe ProcessoComparator diz
		// só preciso pegar o primeiro processo da lista que é o de maior
		// prioridade.
		...
		if (!prioridadesIguais.isEmpty()) {
			...
			Collections.sort(prioridadesIguais, new ProcessoComparator(
	...
	public void executaProcessos() throws Exception {
		...
		// garantia de que quando todos processos forem concluidos o algoritmo
		// ira finalizar aqui.
		if (processo == null) {
			System.out.println("\n\nfim");
			return;
		...
		// se tempo de duração acabou sinal que nao tme mais prioridade e que
		// o procesos finalizou
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < numeroProcesso; i++) {
			...
			circular.adicionar(new Processo(nome, tempoDeChegada,
					tempoExecucao, prioridadeDoProcesso));
